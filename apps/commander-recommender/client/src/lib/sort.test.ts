 ...
 * WUBRG order within a color count, then name, then mana value.
 */
import assert from 'node:assert';
import { describe, it } from 'vitest';
import { sortSuggestions } from './sort';
import { makeCommanderCard, makeSuggestion } from '../test/fixtures';
...
describe('sortSuggestions', () => {
  it('relevance mode is a no-op that returns the same array reference', () => {
    const input = [
  ...
  it('fewest colors sorts before more colors', () => {
    const mono = makeSuggestion({
      cards: [makeCommanderCard({ name: 'Mono' })],
      colorIdentity: ['U'],
    });
    const two = makeSuggestion({
      cards: [makeCommanderCard({ name: 'Two' })],
      colorIdentity: ['U', 'B'],
    });
    const three = makeSuggestion({
      cards: [makeCommanderCard({ name: 'Three' })],
      colorIdentity: ['U', 'B', 'R'],
    });
  ...
  it('same color count sorts by WUBRG order of the identity itself', () => {
    const monoGreen = makeSuggestion({
      cards: [makeCommanderCard({ name: 'Green' })],
      colorIdentity: ['G'],
    });
    const monoWhite = makeSuggestion({
      cards: [makeCommanderCard({ name: 'White' })],
      colorIdentity: ['W'],
    });
    const monoBlack = makeSuggestion({
      cards: [makeCommanderCard({ name: 'Black' })],
      colorIdentity: ['B'],
    });
  ...
  it('two-color identities sort by WUBRG precedence of their first differing color', () => {
    // Orzhov (WB) before Dimir (UB) before Rakdos (BR): W < U < B in WUBRG.
    const orzhov = makeSuggestion({
      cards: [makeCommanderCard({ name: 'Orzhov' })],
      colorIdentity: ['W', 'B'],
    });
    const dimir = makeSuggestion({
      cards: [makeCommanderCard({ name: 'Dimir' })],
      colorIdentity: ['U', 'B'],
    });
    const rakdos = makeSuggestion({
      cards: [makeCommanderCard({ name: 'Rakdos' })],
      colorIdentity: ['B', 'R'],
    });
  ...
  it('same identity sorts alphabetically by name', () => {
    const zed = makeSuggestion({
      cards: [makeCommanderCard({ name: 'Zedruu' })],
      colorIdentity: ['U'],
    });
    const anj = makeSuggestion({
      cards: [makeCommanderCard({ name: 'Anje' })],
      colorIdentity: ['U'],
    });
  ...
  it('a Partner pair sorts by its joined display name', () => {
    const pair = makeSuggestion({
      cards: [
        makeCommanderCard({ name: 'Tymna the Weaver' }),
        makeCommanderCard({ name: 'Thrasios, Triton Hero' }),
      ],
      colorIdentity: ['U'],
    });
    const solo = makeSuggestion({
      cards: [makeCommanderCard({ name: 'Zedruu' })],
      colorIdentity: ['U'],
    });

    const sorted = sortSuggestions([pair, solo], 'colorNameValue');
    assert.deepStrictEqual(sorted.map(unitName), [
      'Tymna the Weaver + Thrasios, Triton Hero',
      'Zedruu',
    ]);
  });

  it('mana value only breaks ties once identity and name already match', () => {
  ...
  it('a Partner pair breaks mana-value ties on its combined cost', () => {
  ...
  it('colorNameValue mode does not mutate the input array', () => {
    const input = [
      makeSuggestion({ cards: [makeCommanderCard({ name: 'B' })] }),
      makeSuggestion({ cards: [makeCommanderCard({ name: 'A' })] }),
    ];
  ...
  it('relevance defaults to descending — the server order, untouched', () => {
    const input = [
      makeSuggestion({ cards: [makeCommanderCard({ name: 'A' })] }),
      makeSuggestion({ cards: [makeCommanderCard({ name: 'B' })] }),
    ];
  ...
  it('relevance ascending reverses the server order', () => {
  ...
  it('colorNameValue descending reverses its natural order', () => {
    const first = makeSuggestion({
      cards: [makeCommanderCard({ name: 'A' })],
      colorIdentity: ['W'],
    });
    const second = makeSuggestion({
      cards: [makeCommanderCard({ name: 'B' })],
      colorIdentity: ['U'],
    });
    assert.deepStrictEqual(sortSuggestions([second, first], 'colorNameValue', 'asc'), [
      first,
      second,
    ]);
    assert.deepStrictEqual(sortSuggestions([second, first], 'colorNameValue', 'desc'), [
      second,
      first,
    ]);
  });

  it('reversing does not mutate the input array', () => {
    const input = [
      makeSuggestion({ cards: [makeCommanderCard({ name: 'A' })] }),
      makeSuggestion({ cards: [makeCommanderCard({ name: 'B' })] }),
    ];
