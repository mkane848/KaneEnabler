...
function ExportControls({ suggestions }: { suggestions: CommanderSuggestionDTO[] }) {
  ...
  return (
    <div className="export-controls">
      <button type="button" className="mtg-btn mtg-btn-ghost export-button" onClick={handleCopy}>
        {copied ? 'Copied!' : 'Copy list'}
      </button>
      <button
        type="button"
        className="mtg-btn mtg-btn-ghost export-button"
...
export function RecommendationResults() {
  ...
  const { brackets, themeFacets, kindredFacets, colors, hasColorless, hasMulticolor } = useMemo(
      ...
      {suggestions.length === 0 ? (
        ...
        <>
          <ResultFilters
            ...
            availableThemeFacets={themeFacets}
            availableKindredFacets={kindredFacets}
