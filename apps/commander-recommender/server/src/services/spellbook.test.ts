...
let findCombos: typeof import('./spellbook').findCombos;
let clearComboCache: typeof import('./spellbook').clearComboCache;
let SpellbookError: typeof import('./spellbook').SpellbookError;

describe('spellbook adapter', () => {
  beforeAll(async () => {
    ...
    // moduleResolution node16; Vite/Vitest resolves it to the .ts source the
    // same way tsx did.
    const mod = await import('./spellbook.js');
    findCombos = mod.findCombos;
    clearComboCache = mod.clearComboCache;
    SpellbookError = mod.SpellbookError;
  });
  ...
  it('normalises nested {card:{name}} / {feature:{name}} shapes', async () => {
    clearComboCache();
    state.requests = [];
          ...
          almostIncluded: [
            {
              id: '999',
              uses: [
                { card: { name: 'Grimgrin, Corpse-Born' } },
    ...
    const result = await findCombos(
      ...
      ['Gravecrawler', 'Basalt Monolith', 'Rings of Brighthearth'],
    );
  ...
  it('tolerates flat string shapes for cards and results', async () => {
    clearComboCache();
    state.handler = (_req, res) =>
      json(res, 200, {
        results: {
  ...
  it('sends an identifying User-Agent and the commander separately', async () => {
    clearComboCache();
    state.requests = [];
    state.handler = (_req, res) =>
      json(res, 200, { results: { included: [], almostIncluded: [] } });
  ...
  it('caches, so a repeat lookup does not hit them again', async () => {
    clearComboCache();
    state.requests = [];
    state.handler = (_req, res) =>
      json(res, 200, { results: { included: [], almostIncluded: [] } });
  ...
  it('cache key ignores card order but respects contents', async () => {
    clearComboCache();
    state.requests = [];
    state.handler = (_req, res) =>
      json(res, 200, { results: { included: [], almostIncluded: [] } });
  ...
  it('surfaces 429 with Retry-After instead of retrying', async () => {
    clearComboCache();
    state.requests = [];
    state.handler = (_req, res) => {
      res.statusCode = 429;
  ...
  it('includes the response body when they reject the call', async () => {
    clearComboCache();
