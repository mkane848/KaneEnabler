 ...
 *
 * The parser is the one piece of this project that silently degrades when it
...
describe('parseCardList', () => {
  for (const testCase of cases) {
    it(testCase.label, () => {
      const actual = parseCardList(testCase.input).map(({ name, quantity }) => ({
        name,
        quantity,
      }));
  ...
  it('keeps the raw field untouched', () => {
