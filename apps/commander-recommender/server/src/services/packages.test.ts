...
import { signalKey, type Role, type SignalCandidate, type SignalMatch } from './signals';

import { analyzeDeck, type DeckAnalysis } from './deckAnalysis';
import {
  attachSuggestions,
  collectionColors,
  requiredSignalKeys,
...
describe('requiredSignalKeys', () => {
  it('cross-archetype slots pull their source archetype into the query set', () => {
  ...
  it('qualified themes are queried with their qualifier, not bare', () => {
    const owned: OwnedCard[] = [];
    const signals = new Map<string, SignalMatch[]>();
    for (let i = 0; i < 4; i++) {
...
describe('attachSuggestions selection', () => {
  it('a short slot is offered cards that play the missing role', () => {
  ...
  it('slots the list already fills get no suggestions', () => {
    const analysis = outletlessDeck();
    const suggested = attachSuggestions(analysis, {
      candidatesByKey: pool([
  ...
  it('cards already in the list are never suggested back', () => {
    const analysis = outletlessDeck();
  ...
  it('a suggestion that would add a color to the deck is dropped', () => {
    ...
    assert.deepStrictEqual(
      suggested.themes[0]!.slots.find((s) => s.key === 'outlet')
  ...
  it('a graveyard-filling slot is offered self-mill cards, not reanimation spells', () => {
    const { owned, signals } = owning(themed('reanimator', 4, ['rewards'], 'r'));
...
describe('attachSuggestions ranking', () => {
  it('a card that also feeds another theme in the list outranks one that does not', () => {
  ...
  it('the theme being filled is not listed as a theme the card "also fits"', () => {
    const analysis = outletlessDeck();
    const suggested = attachSuggestions(analysis, {
  ...
  it('a card doing two jobs in the archetype outranks one doing a single job', () => {
    const analysis = outletlessDeck();
    const suggested = attachSuggestions(analysis, {
      candidatesByKey: pool([
        [
  ...
  it('otherwise, cheaper comes first and ties break by name', () => {
    const analysis = outletlessDeck();
    const suggested = attachSuggestions(analysis, {
      candidatesByKey: pool([
        [
  ...
  it('suggestions are capped so the response stays a shortlist', () => {
    const analysis = outletlessDeck();
...
describe('attachSuggestions restraint', () => {
  it('a complete archetype is left alone', () => {
    const { owned, signals } = owning([
  ...
  it('a missing candidate pool yields no suggestions rather than throwing', () => {
...
describe('collectionColors', () => {
  it('the allowed colors are the union of what the list already plays', () => {
