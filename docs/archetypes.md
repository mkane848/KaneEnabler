...
|        | `storm`                                                                                               | 0      | The one C4 entry with no deck's own confirmed axes naming it — checked directly, not assumed absent. Real Storm-keyword cards exist in the corpus (krenko.txt's Empty the Warrens and Haze of Rage; tenth-doctor-rose-tyler.txt's All of History, All at Once) but none of those decks' own notes claim Storm as an intentional plan — they're there for other reasons (Goblin production, a big flashy spell). **Inferred**, when built: CR 702.39 is a precise, well-defined keyword, so the risk here is closer to `monoColorDevotion`'s than `politics`'s. |
...
- **`burn`'s doubler exclusion is keyed to the actual replacement-effect shape, not the ambiguous
  ...
  in two genuinely different constructions: a doubler's redirect ("if a source *would* deal damage
  ..., it deals that much damage plus N ... *instead*" — Torbran, Thane of Red Fell; Embermaw
...
- **`graveyardToolbox`'s "return ... card ... to your hand" pattern needed an explicit exclusion for
  ...
  **this** card from your graveyard to your hand" (Squee, Goblin Nabob; Adéwalé, Breaker of Chains) and
  ...
  choice among *different* cards — the flexible, many-cards resource this archetype is actually about.
...
- **`powerMatters`'s "power N or greater" phrase needed an explicit exclusion for a blocker
  ...
  describes a *threat* — the size an opponent's blocker would need to be to stop this creature — with
...
- **`monoColorDevotion`'s mono-color scoping is deliberate, not an oversight to "complete" later.**
  ...
  two-color gods correctly produce no signal at all. Nykthos, Shrine to Nyx's "devotion to *that*
