...
|        | `politics`                                                                                            | 1      | yshtola.txt's own confirmed axes name it directly (`"Drain, pillowfort, spellslinger, politics, MV-vs-cost"`) — also corrected from "no deck named". The fuzziest concept in the catalog even so — if it cannot be kept crisp (goad, donate, symmetric effects), drop it rather than ship a vague theme. |
