...
| [`docs/control-primitives-migration.md`](./docs/control-primitives-migration.md) | Execution brief for moving the recommender's remaining bespoke controls onto `@mtg/ui`'s `.mtg-btn`/`.mtg-input`, plus the two platform-theme defects it uncovered — and why most of those controls deliberately stay local (approved, not started) |
