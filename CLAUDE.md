...
- **`packages/*`** are internal (`workspace:*`), unpublished. `packages/config` exists today;
  `docs/handoff.md`'s Phase 2–3 packages (`@mtg/rules`, `@mtg/scryfall`, `@mtg/card-model`,
  `@mtg/mana`, `@mtg/ui`, `@mtg/profile`) land incrementally — check that doc for current status
