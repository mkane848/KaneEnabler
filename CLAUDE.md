...
| [`docs/control-primitives-migration.md`](./docs/control-primitives-migration.md) | Execution brief for moving the recommender's remaining bespoke controls onto `@mtg/ui`'s `.mtg-btn`/`.mtg-input`, plus the two platform-theme defects it uncovered — and why most of those controls deliberately stay local (shipped, with a post-ship verification section worth reading before the next CSS migration) |
...
- A Playwright-launched browser does **not** inherit `$HTTPS_PROXY`; pass it explicitly, with
  `localhost` bypassed, **and** set `PLAYWRIGHT_DISABLE_FORCED_CHROMIUM_PROXIED_LOOPBACK=1` —
  otherwise Playwright routes localhost through the proxy regardless of the bypass. See `TODO.md`'s
  gotcha note — getting this wrong looks exactly like an egress block, and has been misread as one
  before.
